...
|When start button is clicked Menu allowing user to define board settings is loaded| Currently goes straight into a preset game as attribute menu hasnt been created yet| Create an attribute menu and have gameState be set to GAME_SETUP|N|
| When start button is clicked Menu allowing user to define board settings is loaded 2| Loads game setup menu allowing user to enter details and start game | Add load game button to main menu|Y|
...
|User opens all tiles without mines|Game over screen pops up from the bottom saying "Game Over you win" and presents the option to either retry or quit| as stated in "whats supposed to happen"| Add functionality to buttons|Y|
...
|Save game - user cancels without selecting file| File explorer closes and progam stops trying to save|Program crashes with `No such file or directory ''`|Program checks for blank string and if there is one returns false, if there is an exception an error is raised but program doesnt crash| Handle loading files|Y|
